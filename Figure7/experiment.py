#!/usr/bin/env python3
"""Run the Figure 7 experiment; then: python plot.py --results results.csv.

Quick check: python experiment.py --seeds 1 --workers 1 --budgets 620
...
BUDGETS = (140, 200, 260, 300, 340, 380, 420, 460, 500, 560, 620, 680, 740, 800)
PARAMETER = 'e_max_kj'
...
def worker(job):
    ...
    for method in methods:
        ...
        if budget is not None:
            row.update(budget=float(budget), axis='energy')
        rows.append(row)
    return rows
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="uav-figure7-") as directory:
        jobs = []
        for budget in budgets:
            ...
            copy_case(case_path, case_id="nominal" if budget is None else "e" + f"{budget:g}",
