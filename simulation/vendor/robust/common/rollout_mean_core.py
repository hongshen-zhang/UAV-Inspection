"""Numba mean-base companion for the recourse rollout planner."""
from __future__ import annotations
import math
import numpy as np
from numba import njit
from .mte_core import execute_virtual, visit_ok, resource_limits

@njit(cache=True)
def _top3_cluster_mean(i,mask,ft,weights):
    n=len(weights)-1
    a=0.0;b=0.0;c=0.0
...
@njit(cache=True)
def _fast_score_mean(node,t,e,i,mask,ft,fe,weights,deadline,mu,sigma,pars,aa,bb,dd,ee,
    ...
    if dt<=0.0 or de<=0.0: return -1e100
    cmean=math.exp(mu[i]+0.5*sigma[i]*sigma[i])
    ok,et,en,mode,f,ncost=execute_virtual(cmean,dt,de,pars,aa[i],bb[i],dd[i],ee[i],0)
    if not ok: return -1e100
    ...
    score=weights[i]/((cost_floor+cost)**cost_power)
    if cluster_weight!=0.0:
        score+=cluster_weight*weights[i]*_top3_cluster_mean(i,mask,ft,weights)
    return score

@njit(cache=True)
def _fast_choose_mean(node,t,e,mask,ft,fe,weights,deadline,mu,sigma,pars,aa,bb,dd,ee,
                      cost_floor,cost_power,cluster_weight):
    n=len(weights)-1;best_i=0;best=-1e100
    for i in range(1,n+1):
        s=_fast_score_mean(node,t,e,i,mask,ft,fe,weights,deadline,mu,sigma,pars,aa,bb,dd,ee,
                           cost_floor,cost_power,cluster_weight)
...
@njit(cache=True)
def _apply_scenario_task_mean(node,t,e,mask,i,c,ft,fe,weights,deadline,pars,aa,bb,dd,ee):
    at,ae,dt,de=resource_limits(node,t,e,i,ft,fe,deadline,pars)
    bit=np.uint64(1)<<np.uint64(i-1)
    newmask=mask&(~bit)
    if dt<=0.0 or de<=0.0:
...
@njit(cache=True)
def _simulate_candidate_mean(node,t,e,mask,candidate,scenario,ft,fe,weights,deadline,mu,sigma,pars,aa,bb,dd,ee,
                             cost_floor,cost_power,cluster_weight,max_steps):
    sn,st,se,sm,reward=_apply_scenario_task_mean(node,t,e,mask,candidate,scenario[candidate],ft,fe,weights,deadline,pars,aa,bb,dd,ee)
    total=reward
    for _ in range(max_steps):
        if sm==0: break
        i=_fast_choose_mean(sn,st,se,sm,ft,fe,weights,deadline,mu,sigma,pars,aa,bb,dd,ee,
                            cost_floor,cost_power,cluster_weight)
        if i==0: break
        sn,st,se,sm,r=_apply_scenario_task_mean(sn,st,se,sm,i,scenario[i],ft,fe,weights,deadline,pars,aa,bb,dd,ee)
        total+=r
    return total

@njit(cache=True)
def rollout_choose_mean(node,t,e,visited_mask,scenarios,ft,fe,weights,deadline,mu,sigma,pars,aa,bb,dd,ee,
    ...
    used=np.zeros(n+1,dtype=np.uint8);count=0
    for k in range(K):
        bi=0;bs=-1e100
        for i in range(1,n+1):
    ...
    S=scenarios.shape[0];vals=np.empty(S,dtype=np.float64)
    best_i=0;best_q=-1e100;best_fast=-1e100
    for k in range(count):
        i=cand[k]
        for s in range(S):
            vals[s]=_simulate_candidate_mean(node,t,e,mask,i,scenarios[s],ft,fe,weights,deadline,mu,sigma,pars,aa,bb,dd,ee,
                                             cost_floor,cost_power,cluster_weight,max_steps)
        q=0.0
