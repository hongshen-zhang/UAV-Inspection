...
def generate_mission(seed:int,cfg:Config)->Mission:
    ...
    for idx,((lon,lat),r) in enumerate(zip(task_lonlat,regions),start=1):
        ...
        if cfg.deadline_mode=="critical":
            ...
            deadline=float(np.clip(deadline,430.,cfg.t_max-90.))
        else:
            slack=500.+rng.uniform(230.,620.)+(6-urgency)*70.
            deadline=.55*nominal_flight+cfg.deadline_scale*slack
            deadline=float(np.clip(deadline,620.,cfg.t_max-90.))
...
def solve_execution(mission:Mission,state:State,i:int,c:float,force_mode:str='auto')->ExecResult:
    """Pressure-aware MTE-Sub for an actual revealed workload."""
    ...
    q=float(cfg.sub_pressure_q); beta=float(cfg.sub_bottleneck_beta)
    pt=(cfg.t_max/max(dt,1e-12))**q
    pe=(cfg.e_max_kj/max(de,1e-12))**q
    dp=pt+pe
    wt=2.0*pt/dp if dp>0 else 1.0
    we=2.0*pe/dp if dp>0 else 1.0
    def shaped_cost(t,e):
        ut=t/dt; ue=e/de
        ...
        if lo<=hi+1e-12:
            ...
            if energy(fm)<=de+1e-9:
                ...
                if energy(hi)>de:
                    ...
                    hi=a
                a_t=(wt+beta)/dt+we*ph/de; b_t=we*k/de
                f_t=(a_t/(2*b_t))**(1/3) if b_t>1e-30 else lo
                a_e=wt/dt+(we+beta)*ph/de; b_e=(we+beta)*k/de
                f_e=(a_e/(2*b_e))**(1/3) if b_e>1e-30 else lo
                rhs=de/dt-ph; f_k=(rhs/k)**(1/3) if rhs>0 else lo
                a0=wt/dt+we*ph/de; b0=we*k/de
                f0=(a0/(2*b0))**(1/3) if b0>1e-30 else lo
                for f in (lo,hi,fm,f_t,f_e,f_k,f0):
                    f=min(hi,max(lo,f)); t=c/f; en=energy(f)
                    if t<=dt+1e-8 and en<=de+1e-8:
                        rr=ExecResult(True,'local',t,en,f,-1,shaped_cost(t,en))
    ...
    if force_mode in ('auto','mec'):
        for m,(ru,rd,fm) in enumerate(zip(task.ul_rates,task.dl_rates,cfg.mec_cpu_ghz)):
            ...
            t=tu+c/fm+td
            en=cfg.tx_power_w*tu/1000+cfg.rx_power_w*td/1000+cfg.hover_power_w*t/1000
            if t<=dt+1e-9 and en<=de+1e-9:
                rr=ExecResult(True,'mec',t,en,0.,m,shaped_cost(t,en))
                if best is None or rr.normalized_cost<best.normalized_cost-1e-12:best=rr
