...
def _aligned_execution_cost_py(cfg:Config,t:float,e:float,dt:float,de:float)->float:
    ut=t/max(dt,1e-12); ue=e/max(de,1e-12)
    q=cfg.sub_pressure_power; beta=cfg.sub_bottleneck_weight
    if q<=1e-14 and beta<=1e-14:
        return ut+ue
    pt=(cfg.t_max/max(dt,1e-12))**q if q>1e-14 else 1.0
    pe=(cfg.e_max_kj/max(de,1e-12))**q if q>1e-14 else 1.0
    den=pt+pe
            ...
            if energy(fm)<=de+1e-9:
                ...
                for f in (lo,fmid,hi):
                    ...
                    if t<=dt+1e-8 and e<=de+1e-8:
                        cost=_aligned_execution_cost_py(cfg,t,e,dt,de)
    ...
    if force_mode in ('auto','mec'):
        for m,(ru,rd,fm) in enumerate(zip(task.ul_rates,task.dl_rates,cfg.mec_cpu_ghz)):
            ...
            if t<=dt+1e-9 and e<=de+1e-9:
                rr=ExecResult(True,'mec',t,e,0.,m,_aligned_execution_cost_py(cfg,t,e,dt,de))
                if best is None or rr.normalized_cost<best.normalized_cost-1e-12:best=rr
