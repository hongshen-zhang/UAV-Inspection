#!/usr/bin/env python3
"""Run the Figure 5 experiment; then: python plot.py --results results.csv.

Quick check: python experiment.py --seeds 1 --workers 1
...
def worker(job):
    ...
    for method in methods:
        ...
        if budget is not None:
            row.update(budget=float(budget), axis='nominal')
        rows.append(row)
    return rows
...
def main():
    ...
    budgets = BUDGETS
    with tempfile.TemporaryDirectory(prefix="uav-figure5-") as directory:
        jobs = []
